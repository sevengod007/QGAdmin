...
def last_version() -> str:
    return requests.get("https://api.github.com/repos/johIanse/study_xxqg/releases/latest").json().get("tag_name")


def download(github: str, version: str, binaryName: str):
    print("正在下载文件中，请耐心等待！！！")
    content = requests.get(f"{github}/johIanse/study_xxqg/releases/download/{version}/{binaryName}").content
    if platform.system().lower() == "windows":
